...
## Part 1: Collect / generate the data
...
## Extra!
What about those files, `chatbot.ini` and `wsgi.py`, you ask? At one point this project was (still is?) deployed on a Digital Ocean droplet. Here are the tutorials we used to set that up. Although it's the future now, so no guarantee these links still work:
